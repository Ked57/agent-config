...
- Keep portable policy in `policy/`, reusable skills in `skills/`, and shared
  executable logic in `runtime/`. Cursor bridge files must defer to the policy
  packs installed under `.agents/policy/`; do not duplicate policy in `.mdc` files.
